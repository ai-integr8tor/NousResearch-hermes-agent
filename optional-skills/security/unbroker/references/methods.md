...
1. If Hermes has a real page-extraction backend for arbitrary URLs, `web_extract` on the broker
   ...
   If `web_extract` is search-only in this environment (for example DuckDuckGo-backed snippets), do
   not treat it as a page fetcher: go straight to the `site:` probe below, then browser/operator
   checks as needed. Build per-vector URLs from `search.url_patterns` and heed
