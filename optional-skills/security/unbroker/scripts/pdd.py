...
def cmd_fanout(args) -> None:
    ...
    for i, ids in enumerate(grouping["batches"], 1):
        brief = (
            ...
            f"After EVERY record command, run `pdd.py show {args.subject} <broker>` and confirm the "
            f"ledger state/evidence matches what you just wrote; if it does not, retry once and report "
            f"the write failure instead of self-reporting success. "
