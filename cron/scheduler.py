...
def run_job(
    ...
    try:
        ...
        if _cron_per_model is not None:
            ...
            # Global fallback — pass the raw value through: coercing with
            # ``or ""`` turns a YAML boolean False (``reasoning_effort: false``)
            # into "", silently re-enabling thinking for users who disabled it.
            effort = _cfg.get("agent", {}).get("reasoning_effort", "")
