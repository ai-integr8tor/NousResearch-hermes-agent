...
def _should_wrap_cron_response(job: dict, user_cfg: Optional[dict]) -> bool:
    ...
    Precedence:
      1. An explicit per-job ``wrap_response`` bool always wins.
      2. ``no_agent`` jobs default to *unwrapped*. Their script stdout is
      ...
      3. Otherwise fall back to the global ``cron.wrap_response`` (default True).
    """
    job_override = job.get("wrap_response")
    if isinstance(job_override, bool):
        return job_override
    if job.get("no_agent"):
        return False
    try:
