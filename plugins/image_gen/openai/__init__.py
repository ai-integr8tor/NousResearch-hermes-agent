    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if kwargs.get("mask_image"):
            return error_response(
                error=(
                    "The OpenAI API-key image provider does not yet support "
                    "mask_image through Hermes. Use image_gen.provider='openai-codex' "
                    "for GPT-Image mask/inpainting controls."
                ),
        ...
        # ``reference_images`` is kept as a compatibility alias for the
        # GPT-Image controls added before the public schema standardized on
        ...
        for ref in (normalize_reference_images(reference_image_urls) or []):
            sources.append(ref)
        for ref in (normalize_reference_images(kwargs.get("reference_images")) or []):
            sources.append(ref)
