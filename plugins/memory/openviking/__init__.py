            ...
            try:
                ...
                for ctx_type in ("memories", "resources"):
                    ...
                    for item in items[:3]:
                        ...
                        # Search/find API returns empty abstracts for file-level
                        # results (L0/L1 summaries are generated by extraction
                        # pipeline, not by content/write).  Read raw content as
                        # a fallback so the agent receives meaningful context.
                        if not abstract and uri:
                            try:
                                abs_resp = client.get(
                                    "/api/v1/content/read",
                                    params={"uri": uri},
                                )
                                abs_result = abs_resp.get("result", "")
                                if isinstance(abs_result, dict):
                                    abs_result = abs_result.get("content", "")
                                if abs_result:
                                    abstract = str(abs_result)[:256]
                            except Exception:
                                pass
