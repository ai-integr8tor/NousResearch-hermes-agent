...
class LinearAgentAdapter(BasePlatformAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        # first-party @Linear assistant replies inline. When on, send() mirrors
        # the final response as a reply on the mention's source comment (a
        # normal thread that does render) — done adapter-side, not via a prompt
        # directive, because the model reliably confused the source comment
        # with the session's own anchor comment. Default off keeps behavior
        # identical to other third-party agents (e.g. Cursor). create_comments
        # required. UNWIND: if Linear routes a `response` into its sourceComment
        # thread (or otherwise lets third-party agents reply inline), delete
        # this flag, its yaml/env plumbing, the _session_source_targets map, and
        # _maybe_reply_in_source_thread. AgentSession.sourceComment already
        # exists, so this is plausibly server-side work on Linear's part.
        ...
        # session id → (issue_id, source_comment_id) for reply_in_source_thread.
        # Populated at webhook time (the source comment is only in the webhook),
        # consumed in send() (which only has the session id via chat_id).
        self._session_source_targets: dict[str, tuple[str, str]] = {}
    ...
    async def send(
        ...
        try:
            ...
            return SendResult(success=False, error=str(exc))
        await self._maybe_reply_in_source_thread(agent_session_id, content)
        return SendResult(
            success=True,
    ...
    async def _maybe_reply_in_source_thread(self, session_id: str, content: str) -> None:
        """Mirror the final response into the thread the agent was mentioned in.

        Best-effort: the session response is already delivered, so a failure
        here must not fail the send. Fires only when reply_in_source_thread is
        on and this session was spawned from an existing thread (workaround for
        Linear not rendering session responses inline — see the flag comment).
        """
        target = self._session_source_targets.get(session_id)
        if not target:
            return
        issue_id, source_comment_id = target
        try:
            # Linear threads are one level deep: replying must target the
            # thread ROOT, not a reply within it (a reply parentId is rejected
            # with "incorrect parent"). The mention is usually a reply, so
            # resolve to its parent when it has one.
            parent_id = source_comment_id
            source = await self._client.get_comment(source_comment_id)
            root = ((source or {}).get("parent") or {}).get("id")
            if root:
                parent_id = root
        ...
        except Exception as exc:  # noqa: BLE001 - best-effort, response already sent
            logger.warning(
                "[linear_agent] Failed to mirror response into source thread "
                "(comment %s): %s",
                source_comment_id,
                exc,
            )
    ...
    async def handle_webhook(
        ...
        *,
        ...
        # Remember the source comment for this session so send() can mirror the
        # final response into the original thread (see _reply_in_source_thread).
        # Done adapter-side, not via a prompt directive: the prompt also shows
        # the session's own anchor comment id, and the model reliably confused
        # the two — replying on the session comment (invisible) instead of the
        # source comment.
        if (
            self._reply_in_source_thread
            and context.source_comment_id
            and context.issue_id
        ):
            if len(self._session_source_targets) > 512:
                self._session_source_targets.pop(
                    next(iter(self._session_source_targets)), None
                )
            self._session_source_targets[context.agent_session_id] = (
                context.issue_id,
