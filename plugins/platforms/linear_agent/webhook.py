...
def extract_context(
    ...
    return LinearWebhookContext(
        ...
        source_comment_id=_string(
            # AgentSessionWebhookPayload carries a flat `sourceCommentId` on
            # the agentSession object (not top-level, not a nested object).
            session.get("sourceCommentId")
            or payload.get("sourceCommentId")
