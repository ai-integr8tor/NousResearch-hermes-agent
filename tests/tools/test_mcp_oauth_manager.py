...
async def test_handle_401_tracks_inflight_task_to_prevent_gc(tmp_path, monkeypatch):
    ...
    # The done_callback (discard) is scheduled via loop.call_soon once the
    # task completes, so it runs on the next loop tick after handle_401
    # returns. Yield once to let it fire before asserting cleanup.
...
async def test_handle_401_dedup_survives_even_if_task_reference_dropped(tmp_path, monkeypatch):
    ...
    Running it with ``gc.collect()`` mid-flight exercises that window.
    """
    import asyncio
    import gc
