...
class TestDelegateTask(unittest.TestCase):
    ...
    def test_session_cap_releases_budget_on_build_failure(self, mock_run, mock_build):
        ...
        mock_build.side_effect = RuntimeError("child build failed")
        parent = _make_mock_parent()
        parent.session_id = "session-build-fail-test"
        cap = 2
        with patch("tools.delegate_tool._load_config") as mock_cfg:
            mock_cfg.return_value = {"max_children_per_session": cap}
            # First call fails during child construction; the exception
            # propagates and the tool layer is responsible for formatting it.
            ...
            # Now make builds succeed. The previously reserved slots should have
            # been released, so this call fits under the cap.
            mock_build.side_effect = lambda **kwargs: MagicMock()
            result = json.loads(delegate_task(
                goal="After build failure", parent_agent=parent
            ))
            self.assertIn("results", result)
            self.assertEqual(result["results"][0]["status"], "completed")
