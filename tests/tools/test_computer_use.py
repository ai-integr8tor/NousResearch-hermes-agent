...
class TestCuaWindowTargeting:
    class FakeCuaSession:
        def __init__(self, on_screen_windows, all_windows=None, apps=None, structured_apps=None):
            self.on_screen_windows = on_screen_windows
            self.all_windows = all_windows if all_windows is not None else on_screen_windows
            self.apps = apps or []
            self.structured_apps = structured_apps
        ...
        def call_tool(self, name, args, timeout=30.0):
            ...
            if name == "list_apps":
                structured = {"apps": self.structured_apps} if self.structured_apps is not None else None
                return {"data": self.apps, "images": [], "structuredContent": structured, "isError": False}
    ...
    def _backend(self, *, on_screen_windows, all_windows=None, apps=None, structured_apps=None):
        from tools.computer_use.cua_backend import CuaDriverBackend

        backend = CuaDriverBackend()
        backend._session = self.FakeCuaSession(on_screen_windows, all_windows, apps, structured_apps)
        return backend
    ...
    def test_bundle_id_query_uses_system_fallback_when_cua_metadata_omits_bundle_id(self, monkeypatch):
        backend = self._backend(
            on_screen_windows=[
                {"app_name": "Notes", "pid": 1, "window_id": 10, "is_on_screen": True, "z_index": 0},
            ],
            all_windows=[
                {"app_name": "Notes", "pid": 1, "window_id": 10, "is_on_screen": True, "z_index": 0},
                {"app_name": "Antigravity", "pid": 37031, "window_id": 7, "is_on_screen": False, "z_index": 9},
            ],
            apps=[{"name": "- Antigravity", "pid": 37031}],
        )
        calls = []

        def fake_pids_for_bundle_id(bundle_id):
            calls.append(bundle_id)
            return [37031] if bundle_id == "com.google.antigravity" else []

        monkeypatch.setattr(backend, "_system_pids_for_bundle_id", fake_pids_for_bundle_id)

        target = backend._select_window("com.google.antigravity")
        assert target is not None
        assert target["pid"] == 37031
        assert target["window_id"] == 7
        assert calls == ["com.google.antigravity"]

    def test_structured_list_apps_metadata_is_used_for_bundle_id_matching(self):
        backend = self._backend(
            on_screen_windows=[
                {"app_name": "Unknown", "pid": 37031, "window_id": 7, "is_on_screen": True, "z_index": 0},
            ],
            structured_apps=[{"name": "- Browser", "pid": 37031, "bundle_id": "com.example.browser"}],
        )
    ...
    def test_missing_capture_target_clears_previous_active_window(self):
        backend = self._backend(on_screen_windows=[
            {"app_name": "Browser", "pid": 37031, "window_id": 7, "is_on_screen": True, "z_index": 0},
        ])
        assert backend.capture(mode="ax", app="Browser").app == "Browser"
        assert backend._active_pid == 37031

        cap = backend.capture(mode="ax", app="Missing")

        assert cap.app == ""
        assert backend._active_pid is None
        assert backend._active_window_id is None

    def test_missing_focus_target_clears_previous_active_window(self):
        backend = self._backend(on_screen_windows=[
            {"app_name": "Browser", "pid": 37031, "window_id": 7, "is_on_screen": True, "z_index": 0},
        ])
        assert backend.focus_app("Browser").ok
        assert backend._active_pid == 37031

        result = backend.focus_app("Missing")

        assert not result.ok
        assert backend._active_pid is None
        assert backend._active_window_id is None

    def test_identity_substring_match_requires_word_boundaries(self):
        backend = self._backend(on_screen_windows=[
            {"app_name": "Codex", "pid": 1, "window_id": 10, "title": "", "is_on_screen": True, "z_index": 0},
            {"app_name": "Xcode", "pid": 2, "window_id": 20, "title": "", "is_on_screen": True, "z_index": 1},
            {"app_name": "Gmail", "pid": 3, "window_id": 30, "title": "", "is_on_screen": True, "z_index": 2},
        ])

        assert backend._select_window("Code") is None
        assert backend._select_window("Mail") is None

    def test_same_app_prefers_onscreen_window_over_offscreen_titled_window(self):
        backend = self._backend(
            on_screen_windows=[
            ...
            all_windows=[
                {"app_name": "Browser", "pid": 37031, "window_id": 1, "title": "", "is_on_screen": True, "z_index": 0},
                {"app_name": "Browser", "pid": 37031, "window_id": 2, "title": "Documentation", "is_on_screen": False, "z_index": 1},
            ],
        )
