...
def test_legal_framework_eu_codes_map_to_gdpr():
    """Supported EU residency codes must map to gdpr with a national DPA adapter.
    Only catch-all EU / EU-EEA codes are allowed to stay generic."""
    cases = [
        ("EU-IT", "garante"),
        ("EU-FR", "cnil"),
        ("EU-DE", "bfdi"),
        ...
        ("EU-PT", "cnpd_pt"),
        ("EU-RO", "anspdcp_ro"),
        ("EU-DK", "datatilsynet_dk"),
        ("EU-SE", "imy"),
        ("EU-SI", "ip_rs"),
        ("EU-SK", "uoou_sk"),
        ("EEA-IS", "personuvernd_is"),
        ("EEA-LI", "datenschutzstelle_li"),
        ("EEA-NO", "datatilsynet_no"),
        ("UK",    "ico"),
    ]
...
def test_eu_brokers_directory_loads_recursively():
    """The broker loader must walk the eu/ subdirectory — EU-native brokers (Pagine
    Bianche, 118000, etc.) live there, and an EU subject must see them in `next` output."""
    ...
    # 6 EU-native brokers shipped in phase 3
    expected = {"118000", "dastelefonbuch", "tellows", "infobel", "paginebianche", "192_com"}
...
def test_gdpr_scope_filter_returns_only_true_brokers():
    ...
    assert len(scoped) >= 6  # at least the EU-native brokers
    for b in scoped:
...
def test_by_jurisdiction_returns_matching_brokers():
    """by_jurisdiction('EU-IT') must return brokers tagged EU-IT."""
...
def test_by_jurisdiction_eu_returns_eu_native_and_tagged_us_brokers():
    ...
    assert len(broad) >= 6  # at least the EU-native brokers
...
def test_eu_brokers_have_gdpr_in_deletion_kinds():
    ...
    EU_NATIVE = {"118000", "dastelefonbuch", "tellows", "infobel", "paginebianche", "192_com"}
    for b in brokers.load_all():
...
def test_readme_mentions_eu_and_uk_section():
    """The README must explicitly document the EU/EEA/UK path — a regression that drops
    ...
    # the EU/EEA/UK section exists with the right keywords
    assert "EU/EEA/UK" in readme or "EU resident" in readme or "GDPR" in readme
...
def test_dpa_escalation_md_exists_and_references_dpa_command():
    ...
    for dpa in ("Garante", "CNIL", "BfDI", "ICO", "AEPD", "Autoriteit Persoonsgegevens",
                "APD-GBA", "Datatilsynet", "IMY", "UODO", "CPDP Bulgaria", "AZOP",
                "Cyprus Commissioner", "UOOU Czechia", "VDAI", "Personuvernd"):
...
def test_dpa_load_all_returns_curated_adapters():
    ...
    expected = {"garante", "cnil", "ico", "bfdi", "aepd", "ap_nl", "apd_gba",
                "dsb_at", "dpc_ie", "cnpd_pt", "uodo", "imy", "datatilsynet_dk",
                "tietosuoja", "cpdp_bg", "azop_hr", "cpdp_cy", "uoou_cz", "aki_ee",
                "hdpa_gr", "naih_hu", "dvi_lv", "vdai_lt", "cnpd_lu", "idpc_mt",
                "anspdcp_ro", "uoou_sk", "ip_rs", "datatilsynet_no", "personuvernd_is",
                "datenschutzstelle_li"}
    assert expected.issubset(ids), f"missing DPAs: {expected - ids}"
    # sorted by country
...
def test_dpa_for_residency_resolves_eu_codes():
    ...
    cases = [
        ...
        ("EU-PT", "cnpd_pt", "PT"),
        ("EU-RO", "anspdcp_ro", "RO"),
        ("EU-DK", "datatilsynet_dk", "DK"),
        ("EU-SE", "imy",     "SE"),
        ("EU-SI", "ip_rs",   "SI"),
        ("EU-SK", "uoou_sk", "SK"),
        ("EEA-IS", "personuvernd_is", "IS"),
        ("EEA-LI", "datenschutzstelle_li", "LI"),
        ("EEA-NO", "datatilsynet_no", "NO"),
        ("UK",    "ico",     "GB"),
    ]
...
def test_dpas_command_lists_and_resolves_residency():
    ...
    with redirect_stdout(buf):
        pdd.cmd_dpas(ns)
    out = json.loads(buf.getvalue())
    assert out["dpa_adapters"] >= 31
    ids = {a["id"] for a in out["adapters"]}
    assert {"aepd", "ap_nl", "apd_gba", "garante", "ico", "datatilsynet_no",
            "personuvernd_is", "datenschutzstelle_li"}.issubset(ids)
