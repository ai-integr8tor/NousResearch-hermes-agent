...
def _manager(goal: str, *, max_turns: int = 5):
...
def test_premature_give_up_response_overrides_credulous_done_judge(hermes_home):
    mgr = _manager("Create a complete migration plan and implement the safe local changes")
    response = (
    ...
    decision = _evaluate_with_done_judge(mgr, response, "assistant said it could not complete the task")
...
def test_clear_completion_evidence_still_allows_done_verdict(hermes_home):
    ...
    decision = _evaluate_with_done_judge(mgr, response, "deliverable produced and verification passed")
...
def test_specific_actionable_external_blocker_still_allows_done_verdict(hermes_home):
    ...
    decision = _evaluate_with_done_judge(mgr, response, "specific external blocker requires user action")
    assert_done(decision, mgr)
...
def test_specific_missing_data_blocker_still_allows_done_verdict(hermes_home):
    ...
    decision = _evaluate_with_done_judge(mgr, response, "specific missing data file requires user action")
    assert_done(decision, mgr)
...
def test_partial_progress_plus_give_up_still_continues(hermes_home):
    ...
    decision = _evaluate_with_done_judge(mgr, response, "assistant mentioned fixing something")
    assert_continues(decision, mgr)
...
def test_generic_need_more_time_is_not_actionable_blocker(hermes_home):
    ...
    decision = _evaluate_with_done_judge(mgr, response, "assistant asked to continue later")
    assert_continues(decision, mgr)
...
def test_vague_access_permission_blocker_still_continues(hermes_home):
    ...
    decision = _evaluate_with_done_judge(mgr, response, "assistant said it needed access")
    assert_continues(decision, mgr)
...
def test_vague_no_access_blocker_still_continues(hermes_home):
    ...
    decision = _evaluate_with_done_judge(mgr, response, "assistant said it had no access")
    assert_continues(decision, mgr)
