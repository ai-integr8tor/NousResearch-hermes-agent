...
class _FakeLinearClient:
    ...
    async def create_comment(self, issue_id, body, *, parent_id=None, mutation_policy=None):
        self.calls.append(("comment", issue_id, body, parent_id, mutation_policy))
        return {"id": "comment-1"}

    async def get_comment(self, id):
        # The mention is a reply; its thread root is what a reply must target.
        self.calls.append(("get_comment", id))
        return {"id": id, "parent": {"id": "thread-root-1"}}
...
@pytest.mark.asyncio
async def test_reply_in_source_thread_mirrors_final_response_as_reply():
    """With reply_in_source_thread on, a mid-thread mention (Linear supplies a
    source comment) makes send() mirror the final response as a reply on that
    SOURCE comment (the original thread) — not the session's anchor comment,
    and not the model's job. Off (default), it never comments."""
    async def _run(extra):
        ...
        # Real webhook shape: flat sourceCommentId on agentSession.
        ...
        assert status == 200
        # The runner would deliver the final response via send().
        await adapter.send("session-1", "Here are my findings.", metadata={})
    ...
    replies = [c for c in on if c[0] == "comment"]
    assert len(replies) == 1
    # (kind, issue_id, body, parent_id, policy). The reply targets the thread
    # ROOT (resolved from the source comment's parent), not the source comment
    # itself — Linear rejects replying to a reply.
    assert replies[0][1] == "issue-1"
    assert replies[0][2] == "Here are my findings."
    assert replies[0][3] == "thread-root-1"

    off = await _run({})
    assert not any(c[0] == "comment" for c in off)
...
async def test_issue_update_session_replies_as_comment():
    ...
    assert fake_client.calls[-1] == (
        ...
        "issue-1",
        "Reviewed — looks fine.",
        None,  # parent_id: synthetic issue-update replies are top-level
