...
def test_sidecar_patch_failure_is_nonfatal() -> None:
    """The mixed-attachment patch is an enhancement; SDK import remains the hard gate."""
    index = Path("plugins/platforms/photon/sidecar/index.mjs").read_text(
        encoding="utf-8"
    )
    patch_error = index.index("photon-sidecar: spectrum mixed attachment patch failed")
    sdk_import = index.index('await import("spectrum-ts")')
    missing_sdk = index.index("photon-sidecar: spectrum-ts is not installed")

    assert "process.exit" not in index[patch_error:sdk_import]
    assert "process.exit(3);" in index[missing_sdk:]
