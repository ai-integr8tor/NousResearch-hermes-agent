...
class TestGenerate:
    ...
    def test_openrouter_image_api_payload_shape_and_references(self, tmp_path):
        ...
        assert payload["n"] == 1
        assert payload["input_references"][0].startswith("data:image/png;base64,")

    def test_openrouter_image_api_forwards_known_options(self):
        with patch(_RUNTIME, return_value=_runtime_ok()), \
             patch("requests.post", return_value=_mock_image_api_response([_PNG_DATA_URI])) as mock_post, \
             patch("plugins.image_gen.openrouter.save_b64_image", return_value=Path("/tmp/x.png")):
            _openrouter().generate(
    ...
    def test_openrouter_image_api_ignores_invalid_seed(self):
        with patch(_RUNTIME, return_value=_runtime_ok()), \
             patch("requests.post", return_value=_mock_image_api_response([_PNG_DATA_URI])) as mock_post, \
             patch("plugins.image_gen.openrouter.save_b64_image", return_value=Path("/tmp/x.png")):
            _openrouter().generate(prompt="a pet", seed="not-a-number")

        assert "seed" not in mock_post.call_args.kwargs["json"]
