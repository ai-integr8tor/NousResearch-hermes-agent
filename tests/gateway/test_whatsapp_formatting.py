...
class TestWhatsAppGating:
    def test_pairing_group_policy_does_not_silently_drop_groups(self, caplog):
        adapter = _make_adapter()
        adapter._group_policy = "pairing"

        with caplog.at_level(logging.INFO):
            assert adapter._is_group_allowed("120363000000000000@g.us") is True
        assert "no group handshake" in caplog.text

    def test_quoted_text_can_identify_reply_to_bot(self):
        adapter = _make_adapter()
        data = {
            "hasQuotedMessage": True,
            "quotedText": "Earlier answer from 15551234567@s.whatsapp.net",
            "botIds": ["15551234567@s.whatsapp.net"],
        }

        assert adapter._message_is_reply_to_bot(data) is True

    def test_quoted_text_without_bot_identifier_is_not_reply_to_bot(self):
        adapter = _make_adapter()
        data = {
            "hasQuotedMessage": True,
            "quotedText": "Earlier answer from another participant",
            "botIds": ["15551234567@s.whatsapp.net"],
        }
