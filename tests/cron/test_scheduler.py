...
class TestShouldWrapCronResponse:
    ...
    def test_explicit_job_override_wins_over_everything(self):
        # A no_agent job can opt back into wrapping.
        assert _should_wrap_cron_response(
            {"id": "j", "no_agent": True, "wrap_response": True}, None
        ) is True
        # An agent job can opt out even when the global default is on.
        assert _should_wrap_cron_response(
            {"id": "j", "wrap_response": False}, {"cron": {"wrap_response": True}}
        ) is False

    def test_non_bool_job_override_is_ignored(self):
        # A stray non-bool value must not be treated as an override.
        assert _should_wrap_cron_response({"id": "j", "wrap_response": "yes"}, None) is True
