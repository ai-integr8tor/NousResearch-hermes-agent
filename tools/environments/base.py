...
class BaseEnvironment(ABC):
    ...
    def init_session(self):
        ...
        # — but it only exists in bash >= 4.0.  macOS ``/bin/bash`` is 3.2.57
        # (GPLv3 freeze), where ``$BASHPID`` expands to the EMPTY string and
        # every writer degrades to the same literal ``snap.tmp.`` — the tear
        # comes back, worse than ``$$``.  So compute ``__hermes_pid`` with a
        # portable fallback: ``sh -c 'echo $PPID'`` reports the calling
        # (sub)shell's PID (``command -p`` so a clobbered PATH can't hide sh),
        # and ``$$`` as a last resort if even that fails.  The static path is
        # shlex-quoted (Windows/Git-Bash drive letters, spaces) with the PID
        # variable left outside the quotes so it still expands.
        _pid_assign = (
            "__hermes_pid=${BASHPID:-$(command -p sh -c 'echo $PPID' 2>/dev/null)}; "
            "__hermes_pid=${__hermes_pid:-$$}"
        )
        _snap_tmp = shlex.quote(self._snapshot_path + ".tmp.") + "$__hermes_pid"
        bootstrap = (
            f"{_pid_assign}\n"
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # Static path shlex-quoted (Windows/spaces); PID var left to expand.
        _pid_assign = (
            "__hermes_pid=${BASHPID:-$(command -p sh -c 'echo $PPID' 2>/dev/null)}; "
            "__hermes_pid=${__hermes_pid:-$$}"
        )
