...
def _external_dirs_readonly_enabled() -> bool:
    ...
    external_dirs. The user must explicitly opt in to allow modifications
    by setting ``skills.external_dirs_readonly: false`` in config.yaml.
    """
    try:
        from hermes_cli.config import load_config
        cfg = load_config()
        ...
        return True  # fail-safe: default to readonly
...
def _check_external_dirs_readonly(name: str, skill_dir: Path) -> Optional[str]:
    ...
    from agent.skill_utils import is_external_skill_dir

    if is_external_skill_dir(skill_dir):
        return (
