...
def _await_gateway_decision(session_key: str, notify_cb, approval_data: dict,
    ...
    case.  Persistence of an approved choice and building the final
    tool-facing result dict remain the caller's responsibility.
    """
    ...
    # attempt, so nothing should observe it as one.
    ...
    if is_gateway or is_ask:
        ...
        if notify_cb is not None:
            ...
            if decision.get("non_admin_unsupported"):
                return {
                    "approved": False,
                    "message": 'BLOCKED: this action requires approval to run a dangerous command, and only the owner/an admin can grant it -- you are talking to a non-admin user who cannot self-approve. Do NOT retry this command, do NOT rephrase it, and do NOT attempt the same outcome via a different command. Tell the user this action needs the owner to run it or to grant them admin access.',
                    "pattern_key": primary_key,
                    "description": combined_desc,
                    "outcome": "non_admin_unsupported",
                    "user_consent": False,
                }
