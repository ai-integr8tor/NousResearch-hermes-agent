...
def _build_gemini_contents(
    ...
    for msg in messages:
        ...
        gemini_role = "user" if role in ("tool", "function") else _ROLE_MAP_OPENAI_TO_GEMINI.get(role, "user")
        new_parts: List[Dict[str, Any]] = []

        if role == "tool" or role == "function":
            new_parts.append(_translate_tool_result_to_gemini(msg))
        else:
            ...
            if isinstance(tool_calls, list):
                for tc in tool_calls:
                    if isinstance(tc, dict):
                        new_parts.append(_translate_tool_call_to_gemini(tc))

        if not new_parts:
            # Gemini rejects empty parts; skip the turn entirely
            continue

        # Merge consecutive turns of the same role to satisfy Code Assist's
        # strict matching of functionCall and functionResponse counts per turn
        if contents and contents[-1]["role"] == gemini_role:
            contents[-1]["parts"].extend(new_parts)
        else:
            contents.append({"role": gemini_role, "parts": new_parts})
