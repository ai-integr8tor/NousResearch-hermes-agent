...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    def _call_chat_completions():
        ...
        if hasattr(stream, "choices"):
            ...
            # Non-streaming fallback: the response object itself carries
            # headers via ._response or .response (httpx). Capture before
            # returning so rate-limit data is not lost on this path.
            _ns_resp = getattr(stream, "_response", None) or getattr(stream, "response", None)
            agent._capture_rate_limits(_ns_resp)
            agent._capture_credits(_ns_resp)
            return stream
