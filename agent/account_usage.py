...
def _coerce_usage_percent(value: Any) -> Optional[float]:
    """Return a percent value for Anthropic usage payload fields.
    ...
    values in ``[0, 1]`` as fractions and larger values as percentages.
    """
...
def _fetch_anthropic_account_usage() -> Optional[AccountUsageSnapshot]:
    ...
    for limit in payload.get("limits") or []:
        ...
        if not label or label in seen_labels:
            continue
        used = _coerce_usage_percent(limit.get("percent"))
        if used is None:
            continue
        windows.append(
            AccountUsageWindow(
