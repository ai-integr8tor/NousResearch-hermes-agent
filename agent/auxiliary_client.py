...
def resolve_provider_client(
    ...
    if provider == "xai-oauth":
        if raw_codex:
            # xai-oauth raw client (fallback activation): the main agent
            # needs direct responses.stream() access, exactly like the
            # openai-codex branch above. The wrapped aux path below returns
            # a CodexAuxiliaryClient whose translation shim is silently
            # dropped whenever the agent rebuilds its client from
            # _client_kwargs, and _resolve_xai_oauth_for_aux's pool-first
            # path hands back stale tokens with no refresh — both fatal for
            # try_activate_fallback. Resolve a REFRESHED token first.
            if not model:
                logger.warning(
            ...
            try:
                ...
                logger.debug(
                    "xai-oauth raw resolution via auth store failed: %s", _xo_exc)
