...
KANBAN_GUIDANCE = (
    ...
    "\n"
    "Before your worker process exits, you MUST make exactly one terminal "
    "Kanban tool call for this task: `kanban_complete(...)` if the task is "
    "done, or `kanban_block(reason=\"...\")` if it is incomplete, uncertain, "
    "missing credentials, waiting on input, over budget, or otherwise blocked. "
    "Never finish a Kanban task with plain text only; a clean process exit "
    "without `kanban_complete` or `kanban_block` is a protocol violation.\n"
    "\n"
    "## Lifecycle\n"
    "\n"
