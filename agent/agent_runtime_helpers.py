...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Attach a response event hook to capture rate-limit headers from EVERY
    # API response (streaming AND non-streaming).  The streaming path already
    # captures via stream.response, but the non-streaming fallback (when an
    # OpenAI-compatible adapter returns a completed ChatCompletion instead of
    # an SSE iterator) loses header access because the SDK's ChatCompletion
    # object doesn't expose the underlying httpx response.  The event hook
    # fires at the httpx layer regardless of how the SDK wraps the result.
    _http_client = client_kwargs.get("http_client")
    if _http_client is not None and hasattr(_http_client, "event_hooks"):
        def _capture_rate_limit_hook(response):
            """httpx response hook: capture rate-limit headers from every API response."""
            try:
                agent._capture_rate_limits(response)
            except Exception:
                pass
        existing_hooks = _http_client.event_hooks.get("response", [])
        if _capture_rate_limit_hook not in existing_hooks:
            _http_client.event_hooks["response"] = existing_hooks + [_capture_rate_limit_hook]
