"""API-only prompt-view cleanup for conversation assembly."""
...
def compact_stale_payloads_for_prompt(
    ...
    *,
    ...
    """Return a prompt view with stale heavy payloads replaced.

    The canonical transcript is never mutated. A detached copy is created only
    when at least one stale payload actually needs replacement.
    """
    ...
    for idx, msg in enumerate(messages):
        ...
        tool_name = str(
            msg.get("tool_name")
            or msg.get("name")
            or tool_names_by_call_id.get(tool_call_id, "")
        )
...
def _compact_arguments_json(
    arguments: str, *, tool_name: str, preview_chars: int
) -> str:
    placeholder = {
