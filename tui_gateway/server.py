...
def _load_reasoning_config() -> dict | None:
    ...
    # Global fallback — pass the raw value through: coercing with ``or ""``
    # turns a YAML boolean False (``reasoning_effort: false``/``off``/``no``)
    # into "", silently re-enabling thinking for users who turned it off.
