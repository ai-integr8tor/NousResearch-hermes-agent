...
| `gateway/platform_registry.py` | 平台适配器注册与延迟 SDK 加载（`register_deferred`） |
| `plugins/platforms/` | 内置消息平台插件适配器（`<name>/adapter.py`） |
| `gateway/platforms/` | 共享 `BasePlatformAdapter` 与少量旧式直接适配器（Signal、webhook、API server 等） |
