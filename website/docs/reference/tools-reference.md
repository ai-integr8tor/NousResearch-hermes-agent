...
:::note Outbound platform delivery
`send_message` is no longer registered as an agent-callable tool. Use `hermes send` for scripts/manual sends, cron `deliver=...` for scheduled output, or the gateway delivery layer for platform responses.
:::
