...
1. **Automatic transitions** (deterministic, no LLM). Skills unused for `stale_after_days` (30) become `stale`; skills unused for `archive_after_days` (90) are moved to `~/.hermes/skills/.archive/`. Skills with **`use_count` 0** are not marked stale or archived until they are at least `stale_after_days` old (a newly created skill may simply not have matched a trigger yet). Skills referenced by **any cron job** (including paused or disabled jobs) are never auto-transitioned — the scheduler only bumps usage when a job fires, so cron-linked skills are treated like pinned for the deterministic pass. This is the always-on pruning behavior — it runs whenever the curator is enabled, with no aux-model cost.
