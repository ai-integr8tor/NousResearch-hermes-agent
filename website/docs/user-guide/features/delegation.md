...
- Leaf subagents **cannot** call: `delegate_task`, `clarify`, `memory`, or `execute_code`. Orchestrator subagents retain `delegate_task` but still cannot use the other three.
