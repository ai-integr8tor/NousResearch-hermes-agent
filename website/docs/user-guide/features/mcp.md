...
- Screened by the same prompt-injection scanner as MCP tool descriptions; instructions that trip it are withheld from the prompt (the server's tools keep working) with a warning in the logs.
