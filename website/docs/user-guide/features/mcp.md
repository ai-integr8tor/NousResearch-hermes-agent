...
The MCP server reads conversation data from Hermes's SQLite session store (`~/.hermes/state.db`, including the `gateway_routing` table for gateway session keys). It falls back to `~/.hermes/sessions/sessions.json` only on pre-migration installs. A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `hermes send` CLI.
