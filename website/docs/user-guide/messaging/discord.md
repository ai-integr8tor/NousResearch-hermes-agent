...
## Sending Media (`hermes send` + `MEDIA:` tags)

The Discord adapter supports native file uploads for every common media type via gateway delivery, `hermes send`, and inline `MEDIA:/path/to/file` tags emitted by the agent:
...
Discord forum channels (type 15) don't accept direct messages — every post in a forum must be a thread. Hermes auto-detects forum channels and creates a new thread post whenever it needs to send there, so gateway replies, TTS, images, voice messages, and file attachments all work without special handling from the agent.
