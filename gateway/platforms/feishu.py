...
# Markdown table detection / conversion for CardKit v2 rendering.
# Feishu's `md` tag does NOT support table syntax — we detect tables and
# convert them to CardKit v2 `table` components inside interactive cards.
_MARKDOWN_TABLE_RE = re.compile(r"^\|.*\|\n\|[-|: ]+\|", re.MULTILINE)  # kept for backward compat
_MARKDOWN_TABLE_LINE_RE = re.compile(r"^\s*\|(.+)\|\s*$")
_MARKDOWN_TABLE_DIVIDER_RE = re.compile(r"^\s*\|(?:\s*[-:]+\s*\|)+\s*$")


def _parse_markdown_table(text: str) -> List[Dict[str, Any]]:
    """Parse markdown text and extract tables as CardKit v2 table segments.
...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _build_outbound_payload(self, content: str) -> tuple[str, str]:
        # Check for markdown tables first — if found, send as CardKit v2
        # interactive card for real table rendering instead of plain text.
