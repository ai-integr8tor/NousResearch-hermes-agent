        ...
        if config_yaml_path.exists():
            ...
            if _tl_require_mention is not None:
                ...
                if "require_mention" not in _tg_section:
                    ...
                    if not _e("TELEGRAM_REQUIRE_MENTION"):
            ...
            if isinstance(signal_cfg, dict):
                if "require_mention" in signal_cfg and not _e("SIGNAL_REQUIRE_MENTION"):
...
def _apply_env_overrides(config: GatewayConfig) -> None:
    ...
    def _get_env(name: str, default: str = "") -> str:
        """Get env var, respecting profile secret scope in multiplex mode."""
        try:
            from agent.secret_scope import is_multiplex_active, current_secret_scope
            if is_multiplex_active():
                scope = current_secret_scope()
                if scope is not None:
                    # Multiplex active + scope installed: scope is authoritative.
                    # NEVER fall back to os.environ (which has the default profile's values).
                    val = scope.get(name)
                    return val if val is not None else default
                # Multiplex active but no scope — return default silently
                # instead of calling get_secret() which raises UnscopedSecretError.
                return default
        except Exception:
            pass
        return os.getenv(name, default)

    # Alias for all the os.getenv calls below — replaced in bulk
    _e = _get_env
    ...
    telegram_reply_mode = _e("TELEGRAM_REPLY_TO_MODE", "").lower()
    if telegram_reply_mode in {"off", "first", "all"}:
        if Platform.TELEGRAM not in config.platforms:
            config.platforms[Platform.TELEGRAM] = PlatformConfig()
        config.platforms[Platform.TELEGRAM].reply_to_mode = telegram_reply_mode
    
    telegram_fallback_ips = _e("TELEGRAM_FALLBACK_IPS", "")
    ...
    telegram_home = _e("TELEGRAM_HOME_CHANNEL")
    if telegram_home and Platform.TELEGRAM in config.platforms:
        config.platforms[Platform.TELEGRAM].home_channel = HomeChannel(
            platform=Platform.TELEGRAM,
            chat_id=telegram_home,
            name=_e("TELEGRAM_HOME_CHANNEL_NAME", "Home"),
            thread_id=_e("TELEGRAM_HOME_CHANNEL_THREAD_ID") or None,
        )
    
    # Discord
    discord_token = _e("DISCORD_BOT_TOKEN")
    if discord_token:
        discord_config = _enable_from_env(Platform.DISCORD)
        discord_config.token = discord_token
    
    discord_home = _e("DISCORD_HOME_CHANNEL")
    if discord_home and Platform.DISCORD in config.platforms:
        config.platforms[Platform.DISCORD].home_channel = HomeChannel(
            platform=Platform.DISCORD,
            chat_id=discord_home,
            name=_e("DISCORD_HOME_CHANNEL_NAME", "Home"),
            thread_id=_e("DISCORD_HOME_CHANNEL_THREAD_ID") or None,
        )
    
    # Reply threading mode for Discord (off/first/all)
    discord_reply_mode = _e("DISCORD_REPLY_TO_MODE", "").lower()
    ...
    whatsapp_enabled = _e("WHATSAPP_ENABLED", "").lower() in {"true", "1", "yes"}
    whatsapp_disabled_explicitly = _e("WHATSAPP_ENABLED", "").lower() in {"false", "0", "no"}
    ...
    whatsapp_home = _e("WHATSAPP_HOME_CHANNEL")
    if whatsapp_home and Platform.WHATSAPP in config.platforms:
        config.platforms[Platform.WHATSAPP].home_channel = HomeChannel(
            ...
            name=_e("WHATSAPP_HOME_CHANNEL_NAME", "Home"),
            thread_id=_e("WHATSAPP_HOME_CHANNEL_THREAD_ID") or None,
        )
    ...
    whatsapp_cloud_phone_id = _e("WHATSAPP_CLOUD_PHONE_NUMBER_ID")
    whatsapp_cloud_token = _e("WHATSAPP_CLOUD_ACCESS_TOKEN")
    if whatsapp_cloud_phone_id and whatsapp_cloud_token:
        ...
        wa_cloud_app_id = _e("WHATSAPP_CLOUD_APP_ID")
        if wa_cloud_app_id:
            config.platforms[Platform.WHATSAPP_CLOUD].extra["app_id"] = wa_cloud_app_id
        wa_cloud_app_secret = _e("WHATSAPP_CLOUD_APP_SECRET")
        ...
        wa_cloud_waba_id = _e("WHATSAPP_CLOUD_WABA_ID")
        if wa_cloud_waba_id:
        ...
        wa_cloud_verify_token = _e("WHATSAPP_CLOUD_VERIFY_TOKEN")
    ...
    if whatsapp_cloud_home and Platform.WHATSAPP_CLOUD in config.platforms:
        config.platforms[Platform.WHATSAPP_CLOUD].home_channel = HomeChannel(
            ...
            name=_e("WHATSAPP_CLOUD_HOME_CHANNEL_NAME", "Home"),
            thread_id=_e("WHATSAPP_CLOUD_HOME_CHANNEL_THREAD_ID") or None,
        )

    # Slack
    slack_token = _e("SLACK_BOT_TOKEN")
    if slack_token:
    ...
    slack_home = _e("SLACK_HOME_CHANNEL")
    if slack_home and Platform.SLACK in config.platforms:
        config.platforms[Platform.SLACK].home_channel = HomeChannel(
            platform=Platform.SLACK,
            chat_id=slack_home,
            name=_e("SLACK_HOME_CHANNEL_NAME", ""),
            thread_id=_e("SLACK_HOME_CHANNEL_THREAD_ID") or None,
        )
    
    # Signal
    signal_url = _e("SIGNAL_HTTP_URL")
    signal_account = _e("SIGNAL_ACCOUNT")
    if signal_url and signal_account:
        signal_config = _enable_from_env(Platform.SIGNAL)
        signal_config.extra.update({
            "http_url": signal_url,
            "account": signal_account,
            "ignore_stories": _e("SIGNAL_IGNORE_STORIES", "true").lower() in {"true", "1", "yes"},
        })
    signal_home = _e("SIGNAL_HOME_CHANNEL")
    if signal_home and Platform.SIGNAL in config.platforms:
        config.platforms[Platform.SIGNAL].home_channel = HomeChannel(
            platform=Platform.SIGNAL,
            chat_id=signal_home,
            name=_e("SIGNAL_HOME_CHANNEL_NAME", "Home"),
            thread_id=_e("SIGNAL_HOME_CHANNEL_THREAD_ID") or None,
        )

    # Mattermost
    mattermost_token = _e("MATTERMOST_TOKEN")
    if mattermost_token:
        mattermost_url = _e("MATTERMOST_URL", "")
    ...
    mattermost_home = _e("MATTERMOST_HOME_CHANNEL")
    if mattermost_home and Platform.MATTERMOST in config.platforms:
        config.platforms[Platform.MATTERMOST].home_channel = HomeChannel(
            ...
            name=_e("MATTERMOST_HOME_CHANNEL_NAME", "Home"),
            thread_id=_e("MATTERMOST_HOME_CHANNEL_THREAD_ID") or None,
        )

    # Matrix
    matrix_token = _e("MATRIX_ACCESS_TOKEN")
    matrix_homeserver = _e("MATRIX_HOMESERVER", "")
    if matrix_token or _e("MATRIX_PASSWORD"):
        ...
        matrix_user = _e("MATRIX_USER_ID", "")
        if matrix_user:
            matrix_config.extra["user_id"] = matrix_user
        matrix_password = _e("MATRIX_PASSWORD", "")
        if matrix_password:
            matrix_config.extra["password"] = matrix_password
        matrix_e2ee_mode = _e("MATRIX_E2EE_MODE", "").strip().lower()
        matrix_e2ee = (
            matrix_e2ee_mode in ("required", "require", "optional", "prefer", "preferred")
            or _e("MATRIX_ENCRYPTION", "").lower() in ("true", "1", "yes")
        )
        matrix_config.extra["encryption"] = matrix_e2ee
        if matrix_e2ee_mode:
            matrix_config.extra["e2ee_mode"] = matrix_e2ee_mode
        matrix_device_id = _e("MATRIX_DEVICE_ID", "")
        if matrix_device_id:
            matrix_config.extra["device_id"] = matrix_device_id
    matrix_home = _e("MATRIX_HOME_ROOM")
    if matrix_home and Platform.MATRIX in config.platforms:
        config.platforms[Platform.MATRIX].home_channel = HomeChannel(
            platform=Platform.MATRIX,
            chat_id=matrix_home,
            name=_e("MATRIX_HOME_ROOM_NAME", "Home"),
            thread_id=_e("MATRIX_HOME_ROOM_THREAD_ID") or None,
        )

    # Home Assistant
    hass_token = _e("HASS_TOKEN")
    if hass_token:
        if Platform.HOMEASSISTANT not in config.platforms:
            config.platforms[Platform.HOMEASSISTANT] = PlatformConfig()
        config.platforms[Platform.HOMEASSISTANT].enabled = True
        config.platforms[Platform.HOMEASSISTANT].token = hass_token
        hass_url = _e("HASS_URL")
        if hass_url:
            config.platforms[Platform.HOMEASSISTANT].extra["url"] = hass_url

    # Email
    email_addr = _e("EMAIL_ADDRESS")
    email_pwd = _e("EMAIL_PASSWORD")
    email_imap = _e("EMAIL_IMAP_HOST")
    email_smtp = _e("EMAIL_SMTP_HOST")
    ...
    email_home = _e("EMAIL_HOME_ADDRESS")
    if email_home and Platform.EMAIL in config.platforms:
        config.platforms[Platform.EMAIL].home_channel = HomeChannel(
            platform=Platform.EMAIL,
            chat_id=email_home,
            name=_e("EMAIL_HOME_ADDRESS_NAME", "Home"),
            thread_id=_e("EMAIL_HOME_ADDRESS_THREAD_ID") or None,
        )

    # SMS (Twilio)
    twilio_sid = _e("TWILIO_ACCOUNT_SID")
    if twilio_sid:
        if Platform.SMS not in config.platforms:
            config.platforms[Platform.SMS] = PlatformConfig()
        config.platforms[Platform.SMS].enabled = True
        config.platforms[Platform.SMS].api_key = _e("TWILIO_AUTH_TOKEN", "")
    sms_home = _e("SMS_HOME_CHANNEL")
    if sms_home and Platform.SMS in config.platforms:
        config.platforms[Platform.SMS].home_channel = HomeChannel(
            platform=Platform.SMS,
            chat_id=sms_home,
            name=_e("SMS_HOME_CHANNEL_NAME", "Home"),
            thread_id=_e("SMS_HOME_CHANNEL_THREAD_ID") or None,
        )

    # API Server
    api_server_enabled = _e("API_SERVER_ENABLED", "").lower() in {"true", "1", "yes"}
    api_server_key = _e("API_SERVER_KEY", "")
    api_server_cors_origins = _e("API_SERVER_CORS_ORIGINS", "")
    api_server_port = _e("API_SERVER_PORT")
    api_server_host = _e("API_SERVER_HOST")
    if api_server_enabled or api_server_key:
        ...
        api_server_model_name = _e("API_SERVER_MODEL_NAME", "")
        if api_server_model_name:
            config.platforms[Platform.API_SERVER].extra["model_name"] = api_server_model_name

    # Webhook platform
    webhook_enabled = _e("WEBHOOK_ENABLED", "").lower() in {"true", "1", "yes"}
    webhook_port = _e("WEBHOOK_PORT")
    webhook_secret = _e("WEBHOOK_SECRET", "")
    if webhook_enabled:
    ...
    msgraph_webhook_enabled = _e("MSGRAPH_WEBHOOK_ENABLED", "").lower() in {
        "true",
        "1",
        "yes",
    }
    msgraph_webhook_port = _e("MSGRAPH_WEBHOOK_PORT")
    msgraph_webhook_client_state = _e("MSGRAPH_WEBHOOK_CLIENT_STATE", "")
    msgraph_webhook_resources = _e("MSGRAPH_WEBHOOK_ACCEPTED_RESOURCES", "")
    msgraph_webhook_allowed_cidrs = _e(
    ...
    # DingTalk
    dingtalk_client_id = _e("DINGTALK_CLIENT_ID")
    dingtalk_client_secret = _e("DINGTALK_CLIENT_SECRET")
    if dingtalk_client_id and dingtalk_client_secret:
        ...
        dingtalk_home = _e("DINGTALK_HOME_CHANNEL")
        if dingtalk_home:
            config.platforms[Platform.DINGTALK].home_channel = HomeChannel(
                platform=Platform.DINGTALK,
                chat_id=dingtalk_home,
                name=_e("DINGTALK_HOME_CHANNEL_NAME", "Home"),
                thread_id=_e("DINGTALK_HOME_CHANNEL_THREAD_ID") or None,
            )

    # Feishu / Lark
    feishu_app_id = _e("FEISHU_APP_ID")
    feishu_app_secret = _e("FEISHU_APP_SECRET")
    if feishu_app_id and feishu_app_secret:
        ...
        config.platforms[Platform.FEISHU].extra.update({
            ...
            "domain": _e("FEISHU_DOMAIN", "feishu"),
            "connection_mode": _e("FEISHU_CONNECTION_MODE", "websocket"),
        })
        feishu_encrypt_key = _e("FEISHU_ENCRYPT_KEY", "")
        if feishu_encrypt_key:
            config.platforms[Platform.FEISHU].extra["encrypt_key"] = feishu_encrypt_key
        feishu_verification_token = _e("FEISHU_VERIFICATION_TOKEN", "")
        if feishu_verification_token:
            config.platforms[Platform.FEISHU].extra["verification_token"] = feishu_verification_token
        feishu_home = _e("FEISHU_HOME_CHANNEL")
        if feishu_home:
            config.platforms[Platform.FEISHU].home_channel = HomeChannel(
                platform=Platform.FEISHU,
                chat_id=feishu_home,
                name=_e("FEISHU_HOME_CHANNEL_NAME", "Home"),
                thread_id=_e("FEISHU_HOME_CHANNEL_THREAD_ID") or None,
            )

    # WeCom (Enterprise WeChat)
    wecom_bot_id = _e("WECOM_BOT_ID")
    wecom_secret = _e("WECOM_SECRET")
    if wecom_bot_id and wecom_secret:
        ...
        wecom_ws_url = _e("WECOM_WEBSOCKET_URL", "")
        if wecom_ws_url:
            config.platforms[Platform.WECOM].extra["websocket_url"] = wecom_ws_url
        wecom_home = _e("WECOM_HOME_CHANNEL")
        if wecom_home:
            config.platforms[Platform.WECOM].home_channel = HomeChannel(
                platform=Platform.WECOM,
                chat_id=wecom_home,
                name=_e("WECOM_HOME_CHANNEL_NAME", "Home"),
                thread_id=_e("WECOM_HOME_CHANNEL_THREAD_ID") or None,
            )

    # WeCom callback mode (self-built apps)
    wecom_callback_corp_id = _e("WECOM_CALLBACK_CORP_ID")
    wecom_callback_corp_secret = _e("WECOM_CALLBACK_CORP_SECRET")
    if wecom_callback_corp_id and wecom_callback_corp_secret:
        ...
        config.platforms[Platform.WECOM_CALLBACK].extra.update({
            ...
            "agent_id": _e("WECOM_CALLBACK_AGENT_ID", ""),
            "token": _e("WECOM_CALLBACK_TOKEN", ""),
            "encoding_aes_key": _e("WECOM_CALLBACK_ENCODING_AES_KEY", ""),
            "host": _e("WECOM_CALLBACK_HOST", "0.0.0.0"),
            "port": env_int("WECOM_CALLBACK_PORT", 8645),
        })

    # Weixin (personal WeChat via iLink Bot API)
    weixin_token = _e("WEIXIN_TOKEN")
    weixin_account_id = _e("WEIXIN_ACCOUNT_ID")
    if weixin_token or weixin_account_id:
        ...
        weixin_base_url = _e("WEIXIN_BASE_URL", "").strip()
        if weixin_base_url:
            extra["base_url"] = weixin_base_url.rstrip("/")
        weixin_cdn_base_url = _e("WEIXIN_CDN_BASE_URL", "").strip()
        if weixin_cdn_base_url:
            extra["cdn_base_url"] = weixin_cdn_base_url.rstrip("/")
        weixin_dm_policy = _e("WEIXIN_DM_POLICY", "").strip().lower()
        if weixin_dm_policy:
            extra["dm_policy"] = weixin_dm_policy
        weixin_group_policy = _e("WEIXIN_GROUP_POLICY", "").strip().lower()
        if weixin_group_policy:
            extra["group_policy"] = weixin_group_policy
        weixin_allowed_users = _e("WEIXIN_ALLOWED_USERS", "").strip()
        if weixin_allowed_users:
            extra["allow_from"] = weixin_allowed_users
        weixin_group_allowed_users = _e("WEIXIN_GROUP_ALLOWED_USERS", "").strip()
        if weixin_group_allowed_users:
            extra["group_allow_from"] = weixin_group_allowed_users
        weixin_split_multiline = _e("WEIXIN_SPLIT_MULTILINE_MESSAGES", "").strip()
        if weixin_split_multiline:
            extra["split_multiline_messages"] = weixin_split_multiline
        weixin_home = _e("WEIXIN_HOME_CHANNEL", "").strip()
        if weixin_home:
            config.platforms[Platform.WEIXIN].home_channel = HomeChannel(
                platform=Platform.WEIXIN,
                chat_id=weixin_home,
                name=_e("WEIXIN_HOME_CHANNEL_NAME", "Home"),
                thread_id=_e("WEIXIN_HOME_CHANNEL_THREAD_ID") or None,
            )

    # BlueBubbles (iMessage)
    bluebubbles_server_url = _e("BLUEBUBBLES_SERVER_URL")
    bluebubbles_password = _e("BLUEBUBBLES_PASSWORD")
    if bluebubbles_server_url and bluebubbles_password:
        ...
        config.platforms[Platform.BLUEBUBBLES].extra.update({
            ...
            "webhook_host": _e("BLUEBUBBLES_WEBHOOK_HOST", "127.0.0.1"),
            "webhook_port": env_int("BLUEBUBBLES_WEBHOOK_PORT", 8645),
            "webhook_path": _e("BLUEBUBBLES_WEBHOOK_PATH", "/bluebubbles-webhook"),
            "send_read_receipts": _e("BLUEBUBBLES_SEND_READ_RECEIPTS", "true").lower() in {"true", "1", "yes"},
        })
        bluebubbles_require_mention = _e("BLUEBUBBLES_REQUIRE_MENTION")
        ...
        bluebubbles_mention_patterns = _e("BLUEBUBBLES_MENTION_PATTERNS")
    ...
    bluebubbles_home = _e("BLUEBUBBLES_HOME_CHANNEL")
    if bluebubbles_home and Platform.BLUEBUBBLES in config.platforms:
        config.platforms[Platform.BLUEBUBBLES].home_channel = HomeChannel(
            platform=Platform.BLUEBUBBLES,
            chat_id=bluebubbles_home,
            name=_e("BLUEBUBBLES_HOME_CHANNEL_NAME", "Home"),
            thread_id=_e("BLUEBUBBLES_HOME_CHANNEL_THREAD_ID") or None,
        )

    # QQ (Official Bot API v2)
    qq_app_id = _e("QQ_APP_ID")
    qq_client_secret = _e("QQ_CLIENT_SECRET")
    if qq_app_id or qq_client_secret:
        ...
        if qq_home:
            config.platforms[Platform.QQBOT].home_channel = HomeChannel(
                ...
                chat_id=qq_home,
                name=_e("QQBOT_HOME_CHANNEL_NAME") or _e(qq_home_name_env, "Home"),
                thread_id=(
                    _e("QQBOT_HOME_CHANNEL_THREAD_ID")
                    or _e("QQ_HOME_CHANNEL_THREAD_ID")
                    or None
                ),
            )

    # Yuanbao — YUANBAO_APP_ID preferred
    yuanbao_app_id = _e("YUANBAO_APP_ID") or _e("YUANBAO_APP_KEY")
    yuanbao_app_secret = _e("YUANBAO_APP_SECRET")
    if yuanbao_app_id and yuanbao_app_secret:
        ...
        yuanbao_bot_id = _e("YUANBAO_BOT_ID")
        if yuanbao_bot_id:
            extra["bot_id"] = yuanbao_bot_id
        yuanbao_ws_url = _e("YUANBAO_WS_URL")
        if yuanbao_ws_url:
            extra["ws_url"] = yuanbao_ws_url
        yuanbao_api_domain = _e("YUANBAO_API_DOMAIN")
        if yuanbao_api_domain:
            extra["api_domain"] = yuanbao_api_domain
        yuanbao_route_env = _e("YUANBAO_ROUTE_ENV")
        if yuanbao_route_env:
            extra["route_env"] = yuanbao_route_env
        yuanbao_home = _e("YUANBAO_HOME_CHANNEL")
        if yuanbao_home:
            config.platforms[Platform.YUANBAO].home_channel = HomeChannel(
                platform=Platform.YUANBAO,
                chat_id=yuanbao_home,
                name=_e("YUANBAO_HOME_CHANNEL_NAME", "Home"),
                thread_id=_e("YUANBAO_HOME_CHANNEL_THREAD_ID") or None,
            )
        yuanbao_dm_policy = _e("YUANBAO_DM_POLICY")
        if yuanbao_dm_policy:
            extra["dm_policy"] = yuanbao_dm_policy.strip().lower()
        yuanbao_dm_allow_from = _e("YUANBAO_DM_ALLOW_FROM")
        if yuanbao_dm_allow_from:
            extra["dm_allow_from"] = yuanbao_dm_allow_from
        yuanbao_group_policy = _e("YUANBAO_GROUP_POLICY")
        if yuanbao_group_policy:
            extra["group_policy"] = yuanbao_group_policy.strip().lower()
        yuanbao_group_allow_from = _e("YUANBAO_GROUP_ALLOW_FROM")
        if yuanbao_group_allow_from:
            extra["group_allow_from"] = yuanbao_group_allow_from

    # Session settings
    idle_minutes = _e("SESSION_IDLE_MINUTES")
    if idle_minutes:
        try:
    ...
    reset_hour = _e("SESSION_RESET_HOUR")
    if reset_hour:
        try:
    ...
    relay_url_env = _e("GATEWAY_RELAY_URL", "").strip()
    relay_url_yaml = ""
